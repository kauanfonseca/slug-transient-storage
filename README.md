...
- **`tsm_consolidated_long.csv`** — a single flat CSV with everything: the
  ...
  column list.
- **`hydraulics_by_point.csv`** — mean/sd/min/max discharge, water velocity,
  stream depth, reach length and reach width, one row per sampling point
  (`point_id` = stream, or stream_station when a stream has more than one
  station) x `discharge_source` (logger/probe/manual), averaged across
  however many valid campaigns exist for that point/method in `events.csv`.
  Most points only have one campaign per method (so most rows are just that
  one value with `sd = NA`) -- CB, RA and SR are the only points with more
  than one campaign to actually average. A point where every campaign was
  excluded (`flag_discharge_invalid`) still gets a row (`discharge_source =
  NONE_VALID`), never silently dropped -- see `excluded_events_note`.
  Generated by `build_hydraulics_by_point.py` from `events.csv`; edit that,
  never this file.
